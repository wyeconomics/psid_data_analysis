...
**Only state level residence is publicly open.** Finer levels including county, tract, block, zip code, MSA, and CBSA are all restricted. I'll explain the details of applying for the data not before long.
...
#| echo: true
#| error: true
name_fam_df <- list.files(path = indir, pattern = paste(".*wlth",yr,".*\\.rda",sep = ""))
...
-   Submit materials include
    -   CV(of a Dr+)
    -   Research plan(justification of such use)
    -   IRB approval
    -   Data request form
